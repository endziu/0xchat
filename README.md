...
| `TRUSTED_PROXY_IPS` | unset | Comma-separated IPs of your reverse proxy/edge. When the direct peer is in this list, the client IP is taken from the rightmost untrusted `X-Forwarded-For` hop; otherwise `X-Forwarded-For` is ignored |
